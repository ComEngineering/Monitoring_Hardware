...
/**
  ...
  * @param  GPIOx: where x can be (A..F) to select the GPIO peripheral for STM32F0xxdevices
  *                      to select the GPIO peripheral for STM32F0xx devices. 
...
/**
  ...
  * @retval None
  */
void GPIO_WriteBit(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, uint16_t BitVal)
{
  /* Check the parameters */
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));
  assert_param(IS_GET_GPIO_PIN(GPIO_Pin));
