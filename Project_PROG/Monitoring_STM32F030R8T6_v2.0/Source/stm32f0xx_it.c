...
/************************************************************/	
	temp = one_wire_read_byte(PIN_ONE_WIRE_T3);
	if((temp == 850) && count85C[2] < 3){
		...
		/* если обрыв или не подключен датчик */
		if(temp == 0xFFFF){
			R_STATES |=  STATE_DISCON_DT3;
			R_STATES &= ~STATE_CIRCUIT_DT3;
			R_STATES &= ~STATE_CRC_ERR_DT3;
			R_DT3 = 0xFFFF;
		}
		/* если замыкание шины данных на землю */
		else if(temp == 0x01){
		...
		/* иначе сбрасываем все ошибки и обновляем температуру */
			...
			R_STATES &= ~ALL_STATE_DT3;
			R_DT3 = temp;
		}
	}
/************************************************************/	
	temp = one_wire_read_byte(PIN_ONE_WIRE_T4);
	if((temp == 850) && count85C[3] < 3){
		...
		/* если обрыв или не подключен датчик */
		if(temp == 0xFFFF){
			R_STATES |=  STATE_DISCON_DT4;
			R_STATES &= ~STATE_CIRCUIT_DT4;
			R_STATES &= ~STATE_CRC_ERR_DT4;
			R_DT4 = 0xFFFF;
		}
		/* если замыкание шины данных на землю */
		else if(temp == 0x01){
